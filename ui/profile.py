...
def init_profile_db():
    with get_conn() as conn:
        ...
        """)
        # migrate existing table if goal_date column missing
        try:
            conn.execute("ALTER TABLE profile ADD COLUMN goal_date TEXT")
        except Exception:
            pass
        # remove old timeframe_weeks if present (ignore error if not)
        try:
            conn.execute("ALTER TABLE profile ADD COLUMN _migration_done INTEGER DEFAULT 0")
        except Exception:
            pass
...
def weeks_until(goal_date_str: str) -> int:
    """Calculate weeks from today to goal date. Minimum 1 week."""
    try:
        goal = date.fromisoformat(goal_date_str)
        days = (goal - date.today()).days
        return max(1, days // 7)
    except Exception:
        return 12

def calculate_profile(age, gender, height_cm, current_weight_kg,
                       goal_weight_kg, goal_date, activity_level):
    if gender == "Male":
    ...
    timeframe_weeks = weeks_until(goal_date)
    ...
    est_weeks = round(abs(total_kcal_needed) / abs(actual_adjustment * 7), 1) \
                if abs(actual_adjustment) > 0 else timeframe_weeks
    ...
    with get_conn() as conn:
        conn.execute("""
            ...
            (user_id, age, gender, height_cm, current_weight_kg, goal_weight_kg,
             goal_date, activity_level, bmr, tdee, daily_calories,
...
def update_weight(new_weight_kg, user_id: str = "default"):
    ...
    if not profile:
        return None
    goal_date = profile.get("goal_date") or \
    ...
    return save_profile(
        ...
        goal_date, profile["activity_level"],
        user_id=user_id
    )